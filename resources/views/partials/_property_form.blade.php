@if (isset($property))
  ...
  <div class="row">
    ...
    <div class="col-md-4">
      <label for="parkings">Número de parqueaderos</label>
  ...
  <div class="form-group flex-checkboxes">
    @foreach ($features as $feature)
      ...
      @else
        @if (array_has($property->features, $feature->id))
          <div class="checkbox">
            <label>
              <input type="checkbox" name="features[]" checked id="feature-{{ $feature->id }}" value="{{ $feature->id }}"> {{ $feature->name }}
            </label>
  ...
  <input type="text" name="video360" id="video360" placeholder="-xNN-bJQ4vI" value="{{ old('video360', $property->video360) }}">
  ...
  @if (!empty($property->property_images))
    ...
    @endforeach
  @endif
  <!-- Esto hay que automatizarlo, ya que si viene de la creación de un proyecto, por defecto asocia el proyecto
